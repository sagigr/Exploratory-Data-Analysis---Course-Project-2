...
AggBCEmissions <- aggregate(BCEmissions[, 'Emissions'], by = list(BCEmissions$year), sum)
colnames(AggBCEmissions) <- c('Year', 'Emissions')
AggBCEmissions$City <- paste(rep('Baltimore City', 4))
AggLACEmissions <- aggregate(LACEmissions[, 'Emissions'], by = list(LACEmissions$year), sum)
